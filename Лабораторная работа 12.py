    ...
    for i in range(len(text)):
        for j in range(len(text[i])):
            ...
            if text[i][j].isdigit() or (text[i][j] in ":/%+" and expression != ""):
                if expression != "":
                    if not (text[i][j] in ":/%+" and expression[-1] in ":/%+"):
                        expression += text[i][j]
                    else:
...
def main():
    text = [
        ...
        "чуть-чуть так сказать - отход52%52ную? И/ 23+23: 12-32//+4/2 запомни… мечту не исполнить два раза, но одну я 5-2 и пьяным тебе наколдую.",
    ]
